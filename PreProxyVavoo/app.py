...
def create_app():
    ...
    async def proxy_hls_stream(request):
        ...
        # ✅ IMPROVED: Wait for file with exponential backoff
        max_retries = 10
        for retry in range(max_retries):
            if os.path.exists(file_path):
                break
            wait_time = min(0.1 * (2 ** retry), 2.0)  # Max 2s
        ...
        try:
            ...
            if range_header:
                ...
                return web.FileResponse(file_path, headers=headers, chunk_size=64*1024)
        ...
        try:
            ...
            class HFApplication(BaseApplication):
                ...
                def load_config(self):
                    # ✅ IMPROVED: Extended timeouts for WiFi stability
                    self.cfg.set("bind", f"0.0.0.0:{PORT}")
                    self.cfg.set("workers", 1)
                    self.cfg.set("worker_class", "aiohttp.GunicornWebWorker")
                    self.cfg.set("timeout", 300)  # ✅ 5 minutes (was 60s)
                    self.cfg.set("keepalive", 75)  # ✅ 75s keepalive (was 2s)
                    self.cfg.set("graceful_timeout", 120)  # ✅ Graceful shutdown
